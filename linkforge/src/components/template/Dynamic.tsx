...
import {areAllSocialLinksEmpty, generateSocialIcons, Link, PreviewData} from "@/lib/utils"
                ...
                >
                    ...
                    <div className="relative p-4 sm:p-8 space-y-4 sm:space-y-8">
                        ...
                        >
                            {data.u && (
                                <motion.div
                                    className="h-24 w-24 sm:h-32 sm:w-32 rounded-full overflow-hidden ring-4 ring-purple-400 mx-auto"
                            ...
                            {data.b && (
                                <motion.p
                                    className="text-base sm:text-lg mt-2 text-gray-600"
                                    initial={{opacity: 0}}
                                    animate={controls}
                                    custom={2}
                                >
                                        ...
                                        link && (
                                            ...
                                            >
                                                <motion.a
                                                    ...
                                                    whileTap={{scale: 0.8}}
                                                >
                                                    <Icon icon={icon} width={20} height={20} className="text-gray-700"/>
                                                </motion.a>
                                            </motion.div>
                                        )
                                )}
                                ...
                                <AnimatePresence>
                                    {data.ls.map((link: Link, index) => (
                                        ...
                                        >
                                            ...
                                            >
                                                <div
                                                    className="flex-shrink-0 p-3 w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-400 to-purple-500 rounded-lg flex items-center justify-center">
                                                    {link.i && link.i.startsWith('http') ? (
                                                        <img src={link.i} alt={link.l}
                                                ...
                                                <span
                                                    className="font-medium text-sm sm:text-lg text-gray-800">{link.l}</span>
                                            </motion.a>
                                        </motion.li>
                                    ))}
                                </AnimatePresence>
